...
export default function CommentSection({ pageName = 'postPvp' }) {
    ...
    async function load() {
        try {
            const res = await fetch(`http://localhost:3000/comment?pageName=${encodeURIComponent(pageName)}`);
    ...
    async function submitComment() {
        ...
        try {
            const res = await fetch(
                `http://localhost:3000/comment?pageName=${encodeURIComponent(pageName)}`,
                {
                method: 'POST',
